...
from config import API_KEY_DEEPSEEK as CONFIG_API_KEY_DEEPSEEK
from prompt import generate_bouncer_prompt, generate_full_prompt

API_URL = "https://llmapi.paratera.com"
BASE_URL = f"{API_URL}/v1/"
API_KEY_DEEPSEEK = os.getenv("DEEPSEEK_API_KEY", CONFIG_API_KEY_DEEPSEEK)
...
MODEL_BOUNCER = "DeepSeek-V4-Flash"
TEMPERATURE_DEFAULT = 0.1
MAX_TOKENS_DEFAULT = 65536
LOG_DIR = "logs_deepseek"
...
def chat(
    ...
    """Centralized chat completion entrypoint for DeepSeek models."""
    kwargs = {
        "model": model,
        "messages": messages,
        "stream": stream,
    ...
    # In OpenAI format, explicitly request stream_options to capture final usage data
    if stream:
...
def stream_chat_deepseek(
    ...
    """Streaming chat with DeepSeek, yields chunks."""
    response = chat(
        messages=messages,
        model=model,
        stream=True,
...
def deepseek_preset_chat(messages: list) -> dict:
        ...
        def generater():
            ...
            if os.path.exists(meta_path):
                with open(meta_path, "r", encoding="utf-8") as meta_f:
                    ...
                    if meta_data.get("Finish Reason") != "stop":
                        yield {"type": "sensitive", "message": "Sensitive content detected"}
                        
        return generater()
    else: 
        # 1. Start the raw LLM stream
        start_time = time.time()
        try:
            raw_stream = stream_chat_deepseek(messages)
        except BadRequestError as exc:
            print(f"DeepSeek rejected the prompt: {exc}")

            def sensitive_generator():
                yield {"type": "sensitive", "message": "Sensitive content detected"}

            return sensitive_generator()
...
def bouncer_preset_chat(user_query: str) -> dict:
    ...
    try:
        ...
        print(f"DeepSeek bouncer rejected the prompt: {exc}")
        response_dict = {
            "is_valid": False,
            "error_code": "UNKNOWN"
        }

        response_dict["rejection_reason"] = get_rejection_message(
            response_dict.get("error_code", ""),
            corrected_query=response_dict.get("corrected_query", None)
        )
    ...
    # Check usage details specifically for OpenAI's CompletionUsage schema
...
if __name__ == "__main__":
    ...
    event_generator = deepseek_preset_chat(demo_messages)
